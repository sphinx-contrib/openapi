...
@pytest.mark.parametrize(
    ...
    glob.glob(
        os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "examples", "v3.0", "*.yaml"
        )
    ),
)
def test_openapi30_success(
    tmpdir, run_sphinx, spec, render_examples, render_request, group_paths
):
    py.path.local(spec).copy(tmpdir.join("src", "test-spec.yml"))
    run_sphinx(
        "test-spec.yml",
...
@pytest.mark.parametrize(
    ...
    glob.glob(
        os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "examples", "v3.1", "*.yaml"
        )
    ),
)
def test_openapi31_success(tmpdir, run_sphinx, spec, render_examples, render_request):
    py.path.local(spec).copy(tmpdir.join("src", "test-spec.yml"))
    run_sphinx(
        "test-spec.yml",
        options={
