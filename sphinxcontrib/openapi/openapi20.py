...
def _httpresource(endpoint, method, properties, convert):
    ...
    for status, response in responses.items():
        ...
        if "schema" in response:
            yield ""
            for line in convert_json_schema(response["schema"], directive=":>json"):
                yield "{indent}{line}".format(**locals())
            yield ""
        ...
        if type_ == "object" and schema.get("properties"):
            ...
            if name:
                ...
                if schema.get("description", ""):
                    ...
                    if constraints:
                        output.append(
                            (name, "{type_} {name}: {constraints}".format(**locals()))
                        )
                    else:
                        output.append((name, "{type_} {name}:".format(**locals())))
...
def openapihttpdomain(spec, **options):
    if "examples" in options:
        raise ValueError("Rendering examples is not supported for OpenAPI v2.x specs.")
